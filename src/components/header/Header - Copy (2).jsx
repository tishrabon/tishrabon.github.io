...
const Header = ({ stick }) => {
  ...
  return (
    ...
    >
      <button
        className={`${theme === "dark" ? "bg-vsmain text-main" : "bg-vslight text-[var(--lightfont)]"} verticalC h-8 w-16 rounded-md border-none text-[15px]`}
        onClick={toggleTheme}
        ref={themeRef}
      >
        {theme === "dark" ? <BsLightbulbOff /> : <GoSun />}
      </button>
      ...
      <div className="relative">
        ...
        {/*HIDDEN/VISIBLE NAVS*/}
        <nav
          className={`${navpass ? "show-nav" : "hidden"} ${theme === "dark" ? "bg-darknavbg" : "bg-lightnavbg"} navBar absolute right-[-12px] top-10 z-[900] mt-2 w-[248px] rounded-md bg-black p-2 pb-4 sm:top-11 sm:w-[256px]`}
          ref={navMenuRef}
        >
          <ul className="flex flex-col gap-y-2">
            ...
            >
              {theme === "dark"
                ? `devs:now\\status> working on upcoming projects...`
