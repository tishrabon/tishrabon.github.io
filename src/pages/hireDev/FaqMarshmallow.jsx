...
const FaqMarshmallow = () => {
  ...
  return (
    <div className={`verticalS mt-5 gap-10`}>
      ...
      {/*INSTRUCTION*/}
      <div className={`verticalC max-w-[700px] gap-5 rounded-[10px] rubbertapRev p-5`}>
        <IoIosInformationCircle size={50} className="text-lightmain"/>
        <div>
          Here’s the list of questions. Click them to view the answers.
        </div>
      ...
      <div className={`verticalS gap-5`}>
        {FaqData.map((qna, index) => (
          qna.type === 'qna' && (
            <div key={qna.id} className={`rubberbutton rounded-md p-2 cursor-help`}
              onClick={() => {
            ...
            >
              ...
              <div className={`${viewAns.includes(qna.id) ? "" : "hidden"} border-t mx-1 mt-2 py-2`}>
                Answer: {qna.answer}
              </div>
      ...
      <div className={`max-w-[700px] verticalS gap-3 text-sm`}>
        ...
        <p>If you're up for a different experience, switch to terminal mode, my personal favorite, built the way I like it. (Click the left button in the header to switch.)</p>    
      </div>           
